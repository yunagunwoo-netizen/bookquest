...
> 최종 업데이트: 2026-04-30 (오후·저녁 — 🎉 풀 100% + Layer 1·2·3 학습 메커니즘 풀 스택 완성 + 컷신 무료 티어 100%)
...
### 9. ⭐ 백로그 #002 완료 — 라운드 4 (중1·중2·중3 18권 풀 25/25/8 +900 항목)
- C091~C108 (18권) 풀 5/3/0 → 25/25/8
- 18권 × 50항목 = 900 신규
- 사전 회피 dedupe 65건 (라운드 3의 두 배 — 18권 × 8 기존 q와의 비교 확률) → 1건 추가 fix → 0건
  - 65건 중 INTRA-NEW-DUP 7건 + quiz/disc 충돌 58건
  - C107의 disc·deep 마지막 q를 동일 q로 작성한 실수 1건 (C107 deep 변형)
- 톤: 중학생 — 학술 어휘 일부 OK, 진지함과 보편성, 청소년 시민 톤, 사회·경제·법·미디어·환경·과학 깊이
- 8대 지식 영역 골고루 분포: history×3, economy×3, environment×3, media×3, law×3, sociology×3, science×3
- 검증 통과: BOOKS=13, CURRICULUM=108, SEASON2=48, 18권 25/25/8, answer/dup/regression/NULL=0
- app-data.js: 2,169,102 → 2,344,284 bytes (+175,182)
- 🎉 **CURRICULUM 108권 모두 25/25/8 → 풀 25/25/8 합계 169권 / 169권 (100%) 도달!**

---

## ⭐ 라운드 4 이후 추가 작업 (2026-04-30 후반 ~ 저녁) — 11 commits

라운드 4 완료 후 이어진 **학습 메커니즘 Layer 1·2·3 풀 스택 + 안정성 fix 6건 + UX 안내 강화 + 컷신 무료 티어 100% 작업**.

### 10. 진행률 표시 가드 5군데 (`403e6ae`)
- `bookProgress[book.id]`가 undefined일 때 빈 % 자리 표시되던 이슈
- BookDetailScreen·CurriculumScreen·주차 리스트 5군데에 `?? 0` 가드
- 이후 HomeScreen 6주차 카드도 별도 fix

### 11. Firestore db globalThis 노출
- `const db = ...`이 script scope라 Babel script 블록에서 안 보여 `ReferenceError: db is not defined`
- `var db = ... ; window.db = db;`로 변경 → 클라우드 동기화 회복

### 12. Layer 2-2 응답시간 모니터링 + 부드러운 알림 (`f5402d9`)
- 모듈 레벨 `quizAskedAt = {}` Map + `QUIZ_TOO_FAST_MS = 2000` 임계값
- 퀴즈 카드 첫 렌더 시 timestamp 기록, 답 클릭 시 응답시간 계산
- `quizState[key].responseTimeMs` 저장 (Firestore 자동 백업)
- 2초 미만 시 `⏱ X.X초만에 답했어요. 천천히 읽고 답하면 더 잘 풀 수 있어!` 부드러운 안내 (XP 영향 없음)

### 13. 배틀 finalize 중복 호출 가드 (`1189544`)
- `submitInteractiveResult` 두 번 호출 → 서버 409 Conflict
- 모듈 레벨 `_finalizeInteractiveBusy = false` 플래그
- 새 배틀 시작 시 false 리셋, finalize 시작 시 true 세팅 → 두 번째 호출 무시

### 14. 배틀 라운드 다양성 (`db2b9f3` + functions deploy)
- **클라이언트**: `getAvatarKnowledgePack` 안에서 `completedBooks` + `keyPoints` 셔플 + `[다양성 시드: TIMESTAMP]` 추가
- **서버 functions**: `buildInteractivePrompt`에 `★ 다양성 강제` 섹션 추가 (`firebase deploy --only functions`)
- → 배틀 라운드마다 다른 책·다른 시각 보장

### 15. 배틀 UX 안내 보강 (`457d215`, `a7a51df`)
- 미니토론 선택 화면 잔여 칩 + 0회 빨간 박스 + 버튼 disable
- gate·ready 화면 코인 정보 칩 (`🪙 내 코인 N개 / 필요 5개`) + 부족 시 빨간 안내
- `!isAdmin` 예외 (관리자 무제한 테스트)

### 16. Layer 2-3 챕터 시퀀스 양 기반 5묶음 잠금 (`06140eb`)
- 책마다 풀 데이터에 `chapterIdx` 매핑 0건 → 양 기반 점진 잠금으로 우회
- 퀴즈 5개씩 5묶음, 토론 5개씩 5묶음, 심화 4개씩 2묶음
- `unlockedCount = (Math.floor(answered / CHUNK) + 1) * CHUNK` 공식
- 진행 칩: `📚 풀이 진행: 12/25 · 5문제 더 풀면 다음 묶음`
- 컷신 PNG 4명(DASH·BUX·ZERO·NUTS) × 6컷 = 24장 동시 push

### 17. 책 접근 차단 안내 일관성 (`58ce8d4`)
- 4 화면 모두 `🔒 Premium` 배지 통일 (HomeScreen 6주차·작은 그리드·주차 리스트, CurriculumScreen)
- 잠긴 책 카드 opacity 0.7 처리
- 모든 차단 분기 안내 매트릭스 `🟢 강` 도달
- 컷신 BUDDY 6장 동시 push

### 18. ⭐ Layer 3 Spaced Repetition 퀴즈 (`2587a23`)
- `SR_INTERVALS_MS = [1일, 3일, 1주, 3주]` + 졸업 stage 4
- `quizState[key].reviewStage`·`nextDueAt` 추가 (Firestore 자동 백업)
- 정답 → stage +1, 오답 → stage 0 리셋
- 노출 우선순위: 복습 due → 새 q → 졸업 q 제외
- 진행 칩: `🔄 복습 N개 대기   🎓 졸업 X/25`
- 컷신 MUJIN 6장 동시 push

### 19. ⭐ Layer 3 Spaced Repetition 토론·심화 + 컷신 무료 티어 100% (`1f0130b`)
- `discussionStarState[key]`에 `reviewStage`·`nextDueAt` 추가
- **별점 기반** 단계 진행: 3성 = stage +1, 2성 = 유지, 0~1성 = 리셋
- 토론·심화 노출 로직에 SR 우선·졸업 제외 적용
- 진행 칩에 `🔄 복습 N개 대기   🎓 졸업 X/total` 추가
- **컷신 4명(ARTH·MONKI·RIX·SHADOW) × 6컷 = 24장** 동시 push
- 🎁 **무료 티어 컷신 10/10 완성** (DASH·BUX·ZERO·NUTS·BUDDY·MUJIN·ARTH·MONKI·RIX·SHADOW)

---

## 🎓 BookQuest 학습 메커니즘 — 3대 풀 전체 풀 스택 적용

| 풀 | Layer 1 (셔플·used-set) | Layer 2 (응답시간 + 챕터 잠금) | Layer 3 (SR 망각곡선) |
|---|---|---|---|
| 퀴즈 (25개) | ✅ | ✅ 응답시간 모니터링 + 5묶음 잠금 | ✅ 정답 기반 단계 |
| 토론 (25개) | ✅ | ✅ 5묶음 잠금 | ✅ 별점 기반 단계 |
| 심화 (8개) | ✅ | ✅ 4묶음 잠금 | ✅ 별점 기반 단계 |

Anki·SuperMemo급 학습 사이클이 169권 × 3종 풀 × 모든 항목에 완전 적용됨. 진짜 학습 앱 메커니즘 완성.
...
### 🎉 백로그 #002 (CURRICULUM 풀 확장) 완료!
- **CURRICULUM 108권 모두 25/25/8 도달 — 풀 25/25/8 합계 169권/169권 (100%)**
- 라운드 1·2·3·4로 신규 56권 + 교체 2권 = 총 +2,800항목 추가
- 더 이상 풀 5/3/0 대기 중인 책 없음

### 다음 우선순위 — Layer 2·3 작업으로 전환
...
## 데이터 현황 (오늘 세션 끝 시점 — 🎉🎓 풀 100% + 학습 메커니즘 풀 스택 + 컷신 무료 100%)

| 항목 | 풀 상태 |
|---|---|
| **CURRICULUM_BOOKS** | **108권** (학년별 12권 균등) — 모두 25/25/8 |
| **BOOKS (시즌1)** | 13권 모두 25/25/8 |
| **SEASON2 lower / upper / middle** | 각 16권 모두 25/25/8 (총 48권) |
| **CURRENT_SEASON** | **2** (시즌2 라이브 오픈) |
| **app.html** | ~720,526 bytes (Layer 2·3 적용 후, LF endings) |
| **app-data.js** | 2,344,284 bytes (라운드4 후) |
| **풀 25/25/8 합계** | **🎉 169권 / 169권 (100%)** |
| **학습 메커니즘** | **🎓 Layer 1·2·3 풀 스택 — 169권 × 3종 풀 모두 적용** |
| **컷신 무료 티어** | **🎁 10/10 (60장 PNG)** — DASH·BUX·ZERO·NUTS·BUDDY·MUJIN·ARTH·MONKI·RIX·SHADOW |
| **컷신 남은 작업** | 영웅 12 + 전설 10 + 신화 10 + 히든 10 = 42명 (252장) |
...
| 820a0b8 | feat: CURRICULUM 5·6학년 12권 + 교체 2권(C079~C090, C018, C022) 풀 25/25/8 확장 (+700 항목) |
| ce0dfd9 | feat: CURRICULUM 중학생 18권(C091~C108) 풀 25/25/8 확장 (+900 항목) — 169/169 100% 도달 🎉 |
| 403e6ae | fix: 책 상세/카드/주차 리스트 진행률 5군데 가드 — `?? 0` 추가 |
| f5402d9 | feat(layer2): 퀴즈 응답시간 측정 + 2초 미만 부드러운 알림 |
| 1189544 | fix: 배틀 finalize 중복 호출 가드 — submitInteractiveResult 409 방지 |
| db2b9f3 | fix(battle): 라운드 책/키포인트 셔플 + 다양성 시드 + (functions deploy) |
| 457d215 | fix(battle): 미니토론 화면 잔여 기회 표시 + 0회 disable |
| a7a51df | feat(battle): 코인 부족 사전 안내 (코인 칩 + 빨간 박스 + disable) |
| 06140eb | feat(layer2-3): 챕터 시퀀스 — 5개 단위 양 기반 점진 잠금 + 진행 UI + 컷신 4명 |
| 58ce8d4 | feat: 책 접근 차단 안내 일관성 — 4 화면 모두 🔒 Premium + 컷신 BUDDY |
| 2587a23 | 🎓 feat(layer3): Spaced Repetition (망각곡선) 퀴즈 1차 + 컷신 MUJIN |
| **1f0130b** | **🎓 feat(layer3): 토론·심화 SR 확장 — 별점 기반 + 컷신 4명 (무료 티어 100%)** |

**총 14 commit (라운드 1~4 + 라운드 4 이후 11개) + functions deploy 1회 + 컷신 PNG 60장**

---

## 다음 세션 우선순위 (오늘 끝 시점 기준)

### 가벼운 작업 (각 ~10~30분)
- **Functions Node.js 22 업그레이드** — 매 배포마다 deprecation 경고 해소 (2026-10-30 전까지 필수)
- **Layer 2·3 통계 위젯** — 응답시간 평균·SR 진행도 시각화 (데이터 며칠 누적 후 의미↑)
- **챕터 시퀀스 강화** — 시간 + 양 결합 (책 펼친 시간 N분 조건 추가)
- **표지 수집 자동화** — 알라딘 API 연동, 169권 표지 자동 가져오기

### 큰 작업 (각 ~1~3시간)
- **컷신 영웅 12명** — 무료 다음 단계, 워드 v1.4 활용
- **컷신 전설 10 / 신화 10** — Premium 가치 강화
- **컷신 히든 10** — 콜렉션 완성
- **새 기능 검토** — 사용자 피드백 기반

### 운영 작업
- **표지 이미지 수집** (BookQuest_표지수집목록_v2.docx 활용 — 169권 전체)
- **컷신 PNG 시트 자르기** (BookQuest_아바타컷신_프롬프트_v1.4.docx 활용)
- **UI 아이콘 재크롭 + 배경 투명화**
